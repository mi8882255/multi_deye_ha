...
|---|---|---|---|
| `deye-sun-3phase-lv` | SUN-6K, SUN-8K, SUN-10K, SUN-12K | 3-фазный сетевой | 48 |
| `deye-sun-3phase-hv` | SUN-15K, SUN-20K | 3-фазный сетевой HV | 54 |
| `deye-sg05lp3` | SUN-5K÷20K-SG05LP3-EU (вкл. SUN-20K-SG05LP3-EU-SM2) | 3-фазный гибридный | 107 |

### Однофазные

| Модель в конфиге | Инверторы | Тип | Сенсоров |
|---|---|---|---|
| `deye-hybrid-1p` | SUN-5K/6K-SG03LP1-EU, SUN-3.6K÷6K-SG04LP1-EU | 1-фазный гибрид, 2 MPPT | 68 |
| `deye-sg02lp1` | SUN-8K÷16K-SG02LP1-EU, SUN-8K÷16K-SG01LP1-EU | 1-фазный гибрид, 3 MPPT | 71 |
| `deye-micro` | SUN-300G3÷2000G3, SUN-*K-G (строковые) | Микроинвертор / строковый | 26 |
...
### Виртуальные total-сенсоры (`vt_`)

У разных моделей Deye суммарные показатели называются по-разному (`grid_total_power`, `external_ct_total_power`, `dc_total_power` и т.д.) и находятся на разных регистрах. Чтобы не задумываться о модели инвертора, во всех моделях доступны **унифицированные виртуальные сенсоры** с префиксом `vt_`:

| Виртуальный ID | Описание | 3-фаз. сетевой | 3-фаз. гибрид | 1-фаз. гибрид | Микро |
|---|---|:---:|:---:|:---:|:---:|
| `vt_solar_power` | Суммарная мощность PV | + | + | + | + |
| `vt_grid_power` | Суммарная мощность сети | + | + | + | — |
| `vt_load_power` | Суммарное потребление | + | + | + | — |
| `vt_inverter_power` | Суммарная мощность инвертора | + | + | + | + |
| `vt_battery_power` | Мощность батареи | + | + | + | — |
| `vt_generator_power` | Мощность генератора/AUX | — | + | + | — |

Эти сенсоры читают те же регистры, что и оригинальные, но имеют одинаковые ID на всех моделях. Это удобно для:
- **Автоматизаций в HA** — один шаблон работает с любым типом инвертора
- **Мультиинверторных установок** — разные модели, единообразные entity ID
- **Фильтрации сенсоров** — `sensors: [vt_solar_power, vt_grid_power, vt_load_power]` в конфиге

### Справочник сенсоров

Полное описание всех сенсоров для каждой модели (ID, единицы измерения, диапазоны, описание) — см. **[docs/sensors.md](docs/sensors.md)**.
...
**Из GitHub-репозитория (рекомендуется):**

1. В Home Assistant: **Настройки → Дополнения → Магазин дополнений → ⋮ (меню справа вверху) → Репозитории**
2. Вставить URL:
   ```
   https://github.com/mi8882255/multi_deye_ha
   ```
3. Нажать **Добавить** → закрыть
4. Найти **Deye Multi-Inverter Reader** в магазине → **Установить**
5. Настроить на вкладке **Конфигурация**, запустить

Обновления будут приходить автоматически.

**Локальная установка (для разработки):**

1. Скопировать проект на HA-машину:
   ```bash
   scp -r . root@homeassistant:/addons/deye_multi_inverter/
   ```
2. В Home Assistant: **Настройки → Дополнения → Магазин дополнений → ⋮ → Проверить обновления**
3. Появится в разделе «Локальные дополнения» → установить
...
```
├── config.yaml          — конфигурация HA-аддона
├── Dockerfile           — сборка HA-аддона
├── src/
...
Используя данное программное обеспечение, вы подтверждаете, что поддерживаете территориальную целостность и суверенитет Украины в пределах её международно признанных границ.
